...
async function handleSync(sendResponse) {
  try {
    ...
    const settings = await chrome.storage.local.get(['gemini_key', 'jobs']);
    if (!settings.gemini_key) {
      sendResponse({ status: "error", message: "Missing Gemini API Key" });
      return;
    }
    ...
    for (const msg of messages) {
      ...
      const analysis = await GeminiAPI.analyzeEmail(settings.gemini_key, fullEmail);
